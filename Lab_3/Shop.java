...
class Shop {
    ...
    public static Shop initialize(int numOfServers, int numOfSelfChecks, int qMax,
        ...
        for (int j = numOfServers; j < (numOfServers + numOfSelfChecks); j++) {
            tempServers = tempServers.add((j == numOfServers)
                ? new SelfCheckOutServer(j, 0.0, qMax, sharedQueue)
                : new SelfCheckOutServer(j, 0.0, 0, sharedQueue));
        }
    ...
    */
    ...
    public Shop addCustomerToServerQueue(Customer customer, Server serverToCheck) {
        int serverIndex = serverToCheck.getServerIndex();
        Server server = this.servers.get(serverIndex);
        return updateShop(serverIndex, server.addQueue(customer));
    }
    ...
    public Shop removeCustomerFromServerQueue(Server serverToCheck) {
        ...
        if (isSelfCheckOut(server)) {
            server = this.servers.get(numOfServers);
            serverIndex = numOfServers;
        }
        return updateShop(serverIndex, server.deQueue());
    }
    ...
    public int findNearestSelfCheckIndex(Server server) {
        if (isSelfCheckOut(server)) {
            int nearestServerIndex = -1;
            double lowestAvailTime = Double.MAX_VALUE;

            for (Server currServer : this.servers) {
                if (isSelfCheckOut(currServer)) {
                    double availTime = currServer.getAvailableTime();

                    if (availTime < lowestAvailTime) {
                        lowestAvailTime = availTime;
                        nearestServerIndex = currServer.getServerIndex();
                    }
                }
            }
            return nearestServerIndex;
        }
        return server.getServerIndex();
    }

    // find first available server, else return -1
    public int findAvailableServerIndex(double eventTime) {
        for (Server currServer : this.servers) {
            if (currServer.isServerAvail(eventTime)) {
